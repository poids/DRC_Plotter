...
import_data = function(p_df) {
  ...
  if ('Dose_Sum' %in% colnames(p_df)) {
    ...
    #Next three lines change exp_id format form yymmdd to yyyymmdd format (will need to be updated by the year 2021 if this script still in use)
    p_df$exp <- gsub("^18", "2018", p_df$exp)
    p_df$exp <- gsub("^19", "2019", p_df$exp)
    #Converts exp_id into numeric
...
#SUBSET OPTIONS
#Ligand to Show
ligand_to_show='Acetylcholine'

#LigKeys to Show - Not working for some reason
# keys_to_show=c("ach1_1", "ach1_5")

#Constructs to Show
constructs_to_show=c("CODA377", "CODA769", "CODA770", "CODA771", "CODA772", "CODA773", "CODA71")

#Subset Dataframe
sub_df<-df %>%
  filter(ligand==ligand_to_show,
            # key==keys_to_show,
           construct==constructs_to_show
         )
...
if (length(unique(df$construct)) <= 8) {
  ...
  drug_plot_aes=drug_plot +
    xlab(bquote(.(ligand_to_show) ~ "["*mu*M*"]")) +
